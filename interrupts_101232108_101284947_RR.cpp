 ...
 * Round Robin scheduler (100 ms quantum) for SYSC4001 Assignment 3
 */
        ...
        for (auto &proc : job_list) {
            ...
            if ((proc.state == NOT_ASSIGNED || proc.state == NEW)
                ...
                if (assign_memory(proc)) {
                    states old_state = (proc.state == NOT_ASSIGNED ? NEW : proc.state);
                    proc.state = READY;
...
int main(int argc, char** argv) {
    if (argc != 2) {
        std::cout << "ERROR!\nExpected 1 argument, received "
                  << argc - 1 << std::endl;
        std::cout << "To run the program, do: ./interrupts_RR "
